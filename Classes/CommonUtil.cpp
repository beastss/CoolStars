...
std::vector<GoodsData> CommonUtil::getGoodsDatas(const std::string &str)
{
	auto values = parseStrToInts(str.c_str());
	assert(values.size() % 3 == 0);
	std::vector<GoodsData> datas;
	for (size_t i = 0; i < values.size(); i += 3)
	{
		GoodsData data;
		data.type = values[i];
		data.goodsId = values[i + 1];
		data.amount = values[i + 2];
		datas.push_back(data);
	}
	return datas;
}

cocos2d::CCAction *CommonUtil::getScaleAction(bool loop, float scale, float toScaleTime, float toNormalTime)
{
	...
	if (loop)
	{
		seq = CCRepeatForever::create(CCSequence::create(scaleSmall, scaleNormal, NULL));
	}
	else
	{
		seq = CCSequence::create(scaleSmall, scaleNormal, NULL);
	}
	return seq;
}
